...
public class JVMLogParser {
	...
	public static String dataMinuPattern =  ">select .*? from .*? where|insert.*into|update.*set";
	public static String datePattern = "[0-9]{4}[-][0-9]{1,2}[-][0-9]{1,2}";
	public static String timePattern =  ">select .*? from .*? where|insert.*into|update.*set";
	public static String timePattern_2 = "[0-9]{1,2}[:][0-9]{1,2}";
	...
	public static void main(String[] args) {
		...
		//SQL匹配开始---------------------------------
		String matchSql = ">select .*? from .*? where|insert.*into|update.*set";
		...
		try {
			...
			for (int i = 0; i < resultSql.size(); i++) {
				...
				String tmpTimeMin =(String)item.get("timeM");
				//String tmpDate = JVMLogParser.getMatchData(tmpTimeMin, datePattern);
				String tmpDate = "\""+ (String)item.get("timeMin")+"\"";
				String tmpTime = JVMLogParser.getMatchData(tmpTimeMin, timePattern_2);
		...
		try {
			...
			while ((tempString = d.readLine()) != null) {
				...
				if(matchResult!=null) {
					...
					//判断是否定时间
					if(publicLog.sys.isDate)
					{
						if(getMatchData(tempString, "\\["+publicLog.sys.isDateStr)!=null)
						{
							result.add(itemMap);
						}
	         	 ...
	         	 for(int j = i +1 ; j < sourceList.size(); j++){
	             	...
	             	if(timeMin.equals(stimeMin)){
	             		//增加时间循环判定
							...
							if(ec != null) continue;
							String stimeMin_time = (String)subItem.get("time");
							String tmpTime = JVMLogParser.getMatchData(time, timePattern_2);
							String tmpstimeMin_time = JVMLogParser.getMatchData(stimeMin_time, timePattern_2);
							if(tmpTime.equals(tmpstimeMin_time)){
